...
class TestRegisterView(TestCase):
    ...
    """
    The TestRegisterView class has three test methods: test_register_view,
    test_register_view_post and test_register_view_invalid_post.
    The setUp method is used to set up the test data.
    In this case, we create a Client object, set up the URLs for
    the register and login views, and create some test data
    to use in our tests.
    - test_register_view tests that the register view
    returns a 200 status code and uses the correct template.
    - test_register_view_post tests that the register view correctly
    creates a new user and logs them in when valid data is submitted
    via POST request.
    - test_register_view_invalid_post tests that the register view
    correctly handles invalid data submitted via POST request.
    """
    ...
    def test_register_view_invalid_post(self):
        response = self.client.post(
            self.register_url, data=self.invalid_user_data
            )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'accounts/register.html')
...
class TestLoginView(TestCase):
    ...
    """
    The setUp method is used to create a test client, define URLs to test,
    and create a user for testing.
    The TestLoginView class has three test methods:
    - test_login_successful tests that a user can log in successfully
    by sending a POST request to the loginView with correct credentials.
    It asserts that the response status code is 302 (redirect),
    the response redirects to the recipes page, and the _auth_user_id key
    is in the session.
    - test_login_failure tests that the login fails when incorrect credentials
    are sent. It asserts that the response status code is 200 (OK),
    the response contains the error message, and the _auth_user_id key
    is not in the session.
    - test_login_view_uses_correct_template tests that the login view
...
class TestLogoutView(TestCase):
    ...
    """
    The setUp method is used to create the data required by the tests.
    In this case, the test client is created, the logout URL is defined,
    and a test user is created and logged in.
    The TestLogoutView class has one test method:
    - test_logout_successful tests whether the logout view
    returns a 302 (temporary redirect) status code, redirects to the
    homepage (recipes) and that the _auth_user_id key is not in the session.
    """
...
class TestRecipesView(TestCase):
    ...
    - test_recipes_view_uses_correct_template, tests whether the view
    is rendering the correct template.
    - test_recipes_view_pagination, tests whether the pagination
    is working correctly.
    We first test the initial page without any pagination,
    and then we add more recipes to the database to test the pagination
    functionality.
    """
    ...
    def setUp(self):
        ...
        self.recipe1 = Recipe.objects.create(
            creator=self.user, title='Recipe 1',
            short_description='Description 1', ingredients='Ingredients 1',
            instructions='Instructions 1'
            )
        self.recipe2 = Recipe.objects.create(
            creator=self.user, title='Recipe 2',
            short_description='Description 2', ingredients='Ingredients 2',
            instructions='Instructions 2'
            )
        self.recipe3 = Recipe.objects.create(
            creator=self.user, title='Recipe 3',
            short_description='Description 3', ingredients='Ingredients 3',
    ...
    def test_recipes_view_pagination(self):
        ...
        self.assertTrue(
            isinstance(response.context['recipes_list'][0], Recipe)
            )
        self.assertTrue(
            isinstance(response.context['recipes_list'][1], Recipe)
            )
        self.assertTrue(
            isinstance(response.context['recipes_list'][2], Recipe)
            )
        self.assertTrue(
            response.context['recipes_list'].has_previous() is False
            )
        self.assertTrue(
            response.context['recipes_list'].has_next() is False
            )

        # add more recipes
        for i in range(4, 10):
            Recipe.objects.create(
                title='Recipe %d' % i, short_description='Description %d' % i
                )
        ...
        self.assertEqual(
            response.context['recipes_list'].previous_page_number(), 1
            )
        self.assertEqual(
            response.context['recipes_list'].next_page_number(), 3
            )
...
class TestRecipeUnitView(TestCase):
    ...
    """
    We defined the setUp method to create a test user, a test recipe
    and a test comment in the database before each test is run.
    The TestRecipeUnitView class has two test methods:
    - The test_recipe_unit_view method tests that the recipeUnit view
    returns a status code of 200, it uses the recipe.html template
    and that the recipe contains all its attributes
    such as creator, title, etc.
    - The test_add_comment method tests that a comment can be added
    to the recipe using the CommentForm and that the comment
    ...
    def test_add_comment(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertEqual(
            response.url, reverse('recipeunit', args=[self.recipe.id])
            )
...
class TestCreateRecipe(TestCase):
    ...
    """
    The setUp method is used to create the data required by the test.
    In this case, the test client and the recipe are created.
    The TestCreateRecipe class has one test method:
    - test_create_recipe_authenticated, tests that a recipe
    can be created successfully when the user is authenticated, it tests
    that the user is redirected to recipes page after recipe created,
    it tests that the recipe exists, it tests that the recipe creator
    is set to the authenticated user, it tests that the __str__ method
    of the Recipe model returns the recipe title correctly
    ...
    def setUp(self):
        ...
        self.recipe_data = {
            ...
            'ingredients': 'Test Ingredient 1\nTest Ingredient 2\nTest Ingredient 3',  # noqa
            'instructions': 'Test Instructions 1\nTest Instructions 2\nTest Instructions 3',  # noqa
        }

    # Tests that a recipe can be created successfully
    # when the user is authenticated, that the user is
    # redirected to recipes page after recipe created,
    # that the recipe exists, that the recipe creator
    # is set to the authenticated user, and that the __str__ method
    # of the Recipe model returns the recipe title correctly.
    def test_create_recipe_authenticated(self):
        self.client.login(username='testuser', password='testpass')
        response = self.client.post(
            reverse('create-recipe'), data=self.recipe_data
            )
        self.assertEqual(response.status_code, 302)
        ...
        self.assertEqual(
            str(recipe),
            self.recipe_data.get('title') + ' by ' + self.user.username
            )
...
class TestUpdateRecipe(TestCase):
    ...
    """
    We define a setUp method to create a test user and a test recipe
    that we can use in our tests.
    The TestUpdateRecipe class has five test methods:
    - test_update_recipe_form_valid tests that the form is valid
    when given valid data.
    - test_update_recipe_form_invalid tests that the form is invalid
    when given invalid data.
    - test_update_recipe_view tests that it returns a 200 status code
    when accessed via GET.
    - test_update_recipe_view_post tests that it redirects
    to the recipeUnit view with the correct arguments when given
    valid data via POST.
    - test_update_recipe_view_post_invalid tests that it returns
    a 200 status code when given invalid data via POST.
    """
    ...
    def test_update_recipe_view_post(self):
        ...
        self.assertRedirects(
            response, reverse('recipeunit', args=[self.recipe.id])
            )
...
class TestDeleteRecipe(TestCase):
    ...
    - test_delete_recipe_view tests that the view can be accessed
    by a logged-in user with the appropriate permissions,
    that it returns a 200 status code when accessed via GET and
    that the correct template is used for the view.
    - test_delete_recipe_view_post tests whether the delete-recipe view
    can actually delete the recipe when a POST request is made to it,
    it tests that the response status code is 302,
    which indicates a successful redirect,
    that goes to the recipes URL, indicating that the recipe
...
class TestAboutUs(TestCase):
    ...
    - test_about_us_view tests that it returns a 200 status code
    when accessed via GET.
    """
...
class TestGallery(TestCase):
    ...
    - test_gallery_view tests that it returns a 200 status code
    when accessed via GET
...
class TestLikeRecipe(TestCase):
    ...
    - test_like_recipe tests a POST request to the like-recipe URL
    with the ID of the test recipe as a parameter.
    We expect the response status code to be 302 (redirect),
    and the response URL to be the URL of the recipe detail
    page for the test recipe. We also refresh the recipe object
    from the database and assert that the user's like has been added
    to the recipe. We make sure that it correctly removes a like
    from the recipe when accessed via POST and that it redirects
    to the recipe detail page. We also refresh the recipe object
    from the database and assert that the user's like has been
    ...
    def setUp(self):
        ...
        self.recipe = Recipe.objects.create(
            creator=self.user, title='Test Recipe',
    ...
    def test_like_recipe(self):
        self.client.login(username='testuser', password='testpass')
        response = self.client.post(
            reverse('like-recipe', args=[self.recipe.id]),
