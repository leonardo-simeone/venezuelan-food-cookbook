...
class TestRecipeForm(TestCase):
    ...
    """
    The TestRecipeForm class has two test methods: test_tags_field
    and test_meta_model.
    - The test_tags_field method tests that the tags field
    is an instance of MultipleChoiceField, has required attribute
    set to False, has the expected choices and widget.
    - The test_meta_model method tests that the Meta attributes
    are set as expected.
    """
    # Tests if the tags field is an instance of MultipleChoiceField,
    # the required attribute set to False, and that the tags field
    # has the correct choices and widget.
    def test_tags_field(self):
        ...
        self.assertEqual(
            form.fields['tags'].choices, [
                ...
                ('Supper', 'Supper'), ('Dessert', 'Dessert'),
                ('Drink', 'Drink')
                ])
        self.assertIsInstance(
            form.fields['tags'].widget, CheckboxSelectMultiple
            )
...
class TestNewUserForm(TestCase):
    ...
    def test_meta_model(self):
        ...
        self.assertEqual(
            form.Meta.fields, ['username', 'email', 'password1', 'password2']
            )
